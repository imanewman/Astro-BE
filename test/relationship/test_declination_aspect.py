...
def test_calculate_declination_aspects__none():
    ...
    assert relationship.declination_aspect.type is None
    assert relationship.declination_aspect.orb is None
...
def test_calculate_declination_aspects__parallel():
    ...
    assert relationship.declination_aspect.type == AspectType.parallel
    assert relationship.declination_aspect.orb == -1
...
def test_calculate_declination_aspects__contraparallel():
    ...
    assert relationship.declination_aspect.type == AspectType.contraparallel
    assert relationship.declination_aspect.orb == 1
