...
def create_summary(points: Dict[Point, PointSchema]) -> SummarySchema:
    ...
    return SummarySchema(
        ...
        is_day_time=calculate_is_day_time(sun.longitude, asc.longitude)
    )


def calculate_is_day_time(sun_longitude: float, asc_longitude: float) -> bool:
    """
    Returns whether the current points are during the day by looking if the sun is below the horizon.

    :param sun_longitude: The degrees from aries of the sun.
    :param asc_longitude: The degrees from aries of the ascendant, representing the horizon.

    :return: Whether these points are found at day time.
    """

    positive_difference_in_degrees = (sun_longitude - asc_longitude) % 360
